...
public class RandomShip {
    public static Board makeShip() {
        //int[][] grid = new int[10][10];
        Board grid = new Board(); //grid to hold ships
...
public static boolean isLegal(int x, int y, int size, boolean isHori, Board grid) {
    ...
    if(isHori) { //if this is a horizontal ship, check horizontally
        for(int i = startX; i < startX + size + 2; i++) { //loop the ship's size + 2 (an extra space infront and behind)
            for(int j = startY; j < startY + 3; j++) {
                if((i >= 0 && i < 10 && j >= 0 && j < 10) && grid.getBoard()[i][j] != Cell.WATER) { //if there is a non-empty coordinate, this is not legal
                    return false;
                }
            }
        }
    ...
    public static Board cleanLegalCheck(Board grid) {
        for (int i = 0; i < grid.getBoard().length; i++) { //loop through the entire grid, removing all 9s that were added to act as ship spacing
            for (int j = 0; j < grid.getBoard()[i].length; j++) {
                if (grid.getBoard()[i][j] == Cell.MISS) {
                    grid.getBoard()[i][j] = Cell.WATER;
                }
            }
        }
        return grid;
    }
    public static Board addHorizontal(Board grid, int id, int initx, int inity) {
        ...
        for(int i = 0; i < size + 2; i++) {  //if placement won't go out-of-bounds, and is past the size of the ship, place a 9, otherwise, place the ship's id if it is in bounds
            if((i == 0 && initx != 0) || (i == size + 1 && initx < 10 - size)) {
                grid.getBoard()[initx + i - 1][inity] = Cell.MISS;
            }
            else if (i != 0 && i < 11 - size) {
                grid.getBoard()[initx + i - 1][inity] = getType(id);
            }
        }
        for(int i = 0; i < size + 2; i++) { //loop above the ship to add spacing if it won't go out-of-bounds
            if(initx + i - 1 >= 0 && initx + i - 1 < 10 && inity > 0){
                grid.getBoard()[initx + i - 1][inity - 1] = Cell.MISS;
            }
        }
    ...
    public static Board addVertical(Board grid, int id, int initx, int inity) {
        int size = getSize(id); //get the size of the ship
        for(int i = 0; i < size + 2; i++) {
            if((i == 0 && inity != 0) || (i == size + 1 && inity < 10 - size)) { //if placement won't go out-of-bounds, and is past the size of the ship, place a 9, otherwise, place the ship's id if it is in bounds
                grid.getBoard()[initx][inity + i - 1] = Cell.MISS;
            }
            else if (i != 0 && i < 11 - size) {
                grid.getBoard()[initx][inity + i - 1] = getType(id);
            }
        }
        for(int i = 0; i < size + 2; i++) { //loop to the left of the ship to add spacing if it won't go out-of-bounds
            if(inity + i - 1 >= 0 && inity + i - 1 < 10 && initx > 0){
                grid.getBoard()[initx - 1][inity + i - 1] = Cell.MISS;
            }
        }
    ...
    public static Cell getType(int id) {
        if(id == 1) {
